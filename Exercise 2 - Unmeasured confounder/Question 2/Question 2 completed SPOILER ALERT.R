...
inv_var_log_OR_ud <- 1/1     #Inverse variance for the association (on log odds scale) between confounder and disease (corresponding to a variance of 1)
...
model_unm_conf_lash_hi_var <- paste0("model{
...
write.table(model_unm_conf_lash_hi_var, file="model_unm_conf_lash_hi_var.txt", quote=FALSE, sep="", row.names=FALSE,
            col.names=FALSE)
...
bug.out.unm_conf_lash_hi_var <- bugs(data=cyst, 
                      inits=inits, 
                      ...
                      n.thin=1,
                      n.chains=3, 
                      model.file="model_unm_conf_lash_hi_var.txt", 
                      debug=TRUE,
                      DIC=FALSE)
...
library(mcmcplots)
bug.mcmc <- as.mcmc(bug.out.unm_conf_lash_hi_var)
...
estimates_unm_conf_lash_hi_var <- data.frame(rbind(bug.out.unm_conf_lash_hi_var$sims.array[k,1,],bug.out.unm_conf_lash_hi_var$sims.array[k,2,],bug.out.unm_conf_lash_hi_var$sims.array[k,3,]))
...
res_unm_conf_lash_hi_var <- round(
  t(
    apply(X=estimates_unm_conf_lash_hi_var, MARGIN=2, FUN=quantile, probs=c(0.5, 0.025, 0.975))
  ), 
  digits=3)
res_unm_conf_lash_hi_var

# Plot the posterior distributions for the adjusted OR using either lower (blue) or larger variance (black) vs biased OR
plot(density(x=estimates_unm_conf_lash_hi_var$OR_adj), 
     main="Adjusted OR",
     xlab="Value", ylab="Density",
     ylim=c(0,8)
)
# Posterior distribution of the lower variance model
