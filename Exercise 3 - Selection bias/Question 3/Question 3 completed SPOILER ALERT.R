...
#S(Dis+ Exp-)
rval.dis.unexp <- epi.betabuster(mode=0.75, conf=0.95, greaterthan=T, x=0.65)  
...
write.table(model_sel3, file="model_sel3.txt", quote=FALSE, sep="", row.names=FALSE,
            col.names=FALSE)
...
estimates_sel3 <- data.frame(rbind(bug.out.sel3$sims.array[k,1,],bug.out.sel3$sims.array[k,2,],bug.out.sel3$sims.array[k,3,]))
...
res_sel3 <- round(
  t(
    apply(X=estimates_sel3, MARGIN=2, FUN=quantile, probs=c(0.5, 0.025, 0.975))
  ), 
  digits=3)
res_sel3

# Plot the posterior distributions for the adjusted vs the biased OR (red)
plot(density(x=estimates_sel3$OR_adj), 
...
lines(density(estimates_sel2$OR_adj), col="blue")
# Posterior distribution of biased OR
lines(density(estimates_sel3$OR_biased), col="red", lty="dashed")
