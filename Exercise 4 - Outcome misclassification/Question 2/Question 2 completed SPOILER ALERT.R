...
Se <- 0.90             #VALUE FOR SENSITIVITY 
Sp <- 0.97		        #VALUE FOR SPECIFICITY  
...
#===	PRIOR	===#		
			...
			Se <- ", Se,"		    #PRIOR FOR SENSITIVITY
			Sp <- ", Sp,"		    #PRIOR FOR SPECIFICITY
			}")
...
bug.out.mis_non_dif2 <- bugs(data=cyst, 
                     inits=inits, 
...
library(mcmcplots)
bug.mcmc <- as.mcmc(bug.out.mis_non_dif2)
...
estimates_mis_non_dif2 <- data.frame(rbind(bug.out.mis_non_dif2$sims.array[k,1,],bug.out.mis_non_dif2$sims.array[k,2,],bug.out.mis_non_dif2$sims.array[k,3,]))
...
res_mis_non_dif2 <- round(
  t(
    apply(X=estimates_mis_non_dif2, MARGIN=2, FUN=quantile, probs=c(0.5, 0.025, 0.975))
  ), 
  digits=3)
res_mis_non_dif2
